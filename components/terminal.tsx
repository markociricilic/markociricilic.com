...
export default function Terminal({ onQuit }: TerminalProps) {
  ...
  // Function to get the welcome screen content
  const getWelcomeScreen = useCallback(() => {
    return (
      <div className="space-y-2">
        <ImageAsciiLogo />
        <p className="font-mono text-white">Welcome to Marko's portfolio! Type help to see available commands.</p>
      </div>
    )
  }, [])
        ...
        return (
          <div className="space-y-2 text-white">
            ...
            <ul className="space-y-1">
              <li>
                <span className="text-white font-bold">about</span> - Learn about Marko
              </li>
              <li>
...
`}
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (command === "clear") {
      // Instead of clearing everything, reset to welcome screen
      setCommandHistory([
        {
          input: "welcome",
          output: getWelcomeScreen(),
  ...
  const executeCommand = (command: string) => {
    // For mobile: directly execute the command
    if (isMobile) {
      if (command === "clear") {
        // Instead of clearing everything, reset to welcome screen
        setCommandHistory([
    ...
    // For desktop: just set the input
    else {
      setInput(command)
    ...
    <div className="flex flex-col h-full">
      <div className="bg-black border border-white/30 rounded-t-md p-2 flex items-center">
        ...
        <span className="text-sm font-mono text-white">
          marko@terminal ~ {currentSection ? `/${currentSection}` : ""}
        </span>
      </div>
